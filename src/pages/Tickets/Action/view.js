...
const ViewAction =({navigation, route})=>{
    ...
    const onRefresh = React.useCallback(() => {
        setRefresh(true)
        API.actionDetail({ id: route.params.action.id, userid: USER.id }, TOKEN).then((result) => {
            setActions(result.data)
            setImageAction(result.data.image!='' ? JSON.parse(result.data.image):[])

            let d1 = [];
            result.data.image != "" && JSON.parse(result.data.image).map((item, index) => {
                d1.push({
                        ...
                        <View style={{width:'90%'}}>
                            ...
                            <View style={styles.baseBoxShadow} >
                                <View style={styles.boxShadow} >
                                    ...
                                    <DataView title='Pegawai' txt={actions.staff.name ? actions.staff.name : 'kosong'}/>
                                    <DataView title='Departemen' txt={actions.dapertement.name}/>
                                    ...
                                    <TouchableHighlight onPress ={() =>{ if(images.length > 0 ){setShowImage(true);console.log(images)};}}>
