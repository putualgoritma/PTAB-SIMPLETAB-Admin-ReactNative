...
const Action = ({navigation, route}) => {
  ...
  const onRefresh = React.useCallback(() => {
    setRefreshing(true);
    API.actions({ticket_id: route.params.ticket.id, userid: USER.id}, TOKEN)
      .then(result => {
              ...
              actions.map((item, index) => {
                ...
                var imagefoto =
                  item.image != ''
                    ...
                    : null;
                var imagefotoDone =
                  item.image_done != null
                    ? (imagefotoDone = JSON.parse(item.image_done)[0])
                    : null;
                      ...
                      <View
                        ...
                        <View
                          ...
                          {Permission.includes('action_show') && (
                            <BtnDetail
                              onPress={() =>
                                navigation.navigate('ViewAction', {
                                  action: item,
                                })
                              }
                            />
                          ...
                          {item.status != 'close' &&
                            Permission.includes('action_show') && (
                              <BtnStaff
                                onPress={() =>
                                  navigation.navigate('StaffAction', {
                                    action_id: item.id,
                                  })
                                }
                              />
                          ...
                          {item.status != 'close' &&
                            Permission.includes('action_staff_edit') && (
                              <BtnEditStatus
                                onPress={() =>
                                  navigation.navigate('EditActionStatus', {
                                    item: item,
                                  })
                                }
                              />
