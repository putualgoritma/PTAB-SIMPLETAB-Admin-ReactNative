...
import {View,ImageBackground,StyleSheet,ScrollView,Image,Text,Modal,TouchableHighlight,RefreshControl} from 'react-native'
...
const ViewTicket =({navigation, route})=>{
    ...
    const onRefresh = React.useCallback(() => {
        setRefresh(true)
        API.ticketDetail({ id: route.params.ticket.id, userid: USER.id }, TOKEN).then((result) => {
...
result.fotopengerjaan != null && result.fotopengerjaan != null && result.fotopengerjaan.map((item, index) => {
          d1.push({
 ...
 <View style={{width:'90%'}}>
         ...
         <View>
             ...
             {ticket.video !='' && 
                 ...
                 <View style={{height : 250, width :'100%'}}>
                     ...
                     {ticket.video !='' && 
                         <VideoPlayer
                             src={{uri :  `https://simpletabadmin.ptab-vps.com` + `${String(ticket.video).replace('public/', '')}` }}
     ...
     {ticket.status !='pending' && 
         <View>
                 ...
                 <TouchableHighlight onPress ={fotosebelum != null ? () =>{ setShowImagePrework(true);} : null}>
                     <ImageBackground source={require('../../../assets/img/ImageLoading.gif') } style={{ height : 220, width : 280}} >
                         <Image 
                             style={{height : 220, width : 280, marginVertical : 10}} 
                             source = {{uri : `https://simpletabadmin.ptab-vps.com` + `${String(fotosebelum).replace('public/', '')}?time="${new Date()}`}}
                         />
                     </ImageBackground>
                 </TouchableHighlight>
         ...
         <View>
             ...
             <View style={{width:'90%'}}>
                 <TouchableHighlight onPress ={fotopengerjaan != null ? () =>{ setShowImagePengerjaan(true);} : null}>
                 <ScrollView style={{flexDirection:'row',}}horizontal={true}>
                 <ImageBackground source={require('../../../assets/img/ImageLoading.gif') } style={{ height : 220, width : 280}} >
                     {fotopengerjaan && fotopengerjaan.map((item,index) => {
                             return (
                                 <View  key={index}  style={{marginVertical:5}}>
     ...
     {ticket.status =='close' &&
         <View>
             ...
             <View style={{width:'90%'}}>
                 <TouchableHighlight onPress ={fotoselesai != null ? () =>{ setShowImageDone(true);} : null}>
                 <ScrollView style={{flexDirection:'row',}}horizontal={true}>
                 <ImageBackground source={require('../../../assets/img/ImageLoading.gif') } style={{ height : 220, width : 280}} >
                     {fotoselesai && fotoselesai.map((item,index) => {
                             return (
                                 <View  key={index} style={{marginVertical:5}}>
