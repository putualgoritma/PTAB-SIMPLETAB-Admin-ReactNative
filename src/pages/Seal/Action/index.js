...
const ActionSeal = ({ navigation, route }) => {
    ...
    const onRefresh = React.useCallback(() => {
        setRefreshing(true);
        API.actionslock({ lockaction_id: route.params.ticket.id}, TOKEN).then((result) => {
    ...
    const handleDelete = ($id) => {
        setLoading(true)
        API.lockactionsDelete($id, TOKEN).then((result) => {
            // console.log(result);
                    ...
                    <View style={{ width: '90%' }}>
                        ...
                        {Permission.includes('action_create') &&
                            <BtnAdd
                                ...
                                width='60%'
                                icon={faPlusCircle}
                                onPress={() => navigation.navigate('AddActionSeal', { ticket: route.params.ticket })}
                            />
                        }
